#ifndef CONST_VALUES_H
#define CONST_VALUES_H

#include <string>

const std::string SEPERATOR = "?";
const std::string POST_COMMAND = "POST";
const std::string GET_COMMAND = "GET";
const std::string DELETE_COMMAND = "DELETE";
const std::string SIGNUP = "signup";
const std::string TRIPS = "trips";
const std::string COST = "cost";
const std::string ACCEPT = "accept";
const std::string FINISH = "finish";
const std::string USERNAME = "username";
const std::string ROLE = "role";
const std::string ORIGIN = "origin";
const std::string DESTINATION = "destination";
const std::string ID = "id";
const std::string PASSENGER_ROLE = "passenger";
const std::string DRIVER_ROLE = "driver";
const std::string IN_HURRY = "in_hurry";
const std::string SORT_BY_COST = "sort_by_cost";
const std::string BAD_REQUEST_ERROR = "Bad_Request";
const std::string PERMISSION_DENIED_ERROR = "Permission_Denied";
const std::string EMPTY_ERROR = "EMPTY";
const std::string NOT_FOUND_ERROR = "Not_Found";
const std::string OK = "OK";
const std::string WAITING_STATUS = "waiting";
const std::string TRAVELING_STATUS = "traveling";
const std::string FINISHED_STATUS = "finished";
const int YES = 1;
const int NO = 0;
const double MAP_SCALE = 110.5;
const double PRICE_CONSTANT = 10000;
const double IN_HURRY_COEFFICIENT = 1.2;

#endif