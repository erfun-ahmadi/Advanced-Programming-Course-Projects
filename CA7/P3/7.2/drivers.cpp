...
vector <Trip*> Driver::show_all_trips(vector <Trip*> trips, string sorted_by_cost) {
    vector <Trip*> wanted_trips;
    if (trips.size() == 0) {
        throw Error(EMPTY_ERROR);
    }
    else if (sorted_by_cost == "no") {
        wanted_trips = show_trips_not_sorted_by_cost(trips);
    }
    else {
        wanted_trips = show_trips_sorted_by_cost(trips);
    }
    return wanted_trips;
}

vector <Trip*> Driver::show_trips_not_sorted_by_cost(vector <Trip*> trips) {
    ...
    for (int num_of_trip = 0; num_of_trip < trips.size(); num_of_trip++) {
        if (trips[num_of_trip]->get_availability() == YES) {
            available_trips.push_back(trips[num_of_trip]);
        }
    }
    return available_trips;
}

vector <Trip*> Driver::show_trips_sorted_by_cost(vector <Trip*> trips) {
    vector <Trip*> available_trips;
    for (int num_of_trip1 = 0; num_of_trip1 < trips.size(); num_of_trip1++) {
        if (trips[num_of_trip1]->get_availability() == YES) {
            for (int num_of_trip2 = num_of_trip1+1; num_of_trip2 < trips.size(); num_of_trip2++) {
                if (trips[num_of_trip2]->get_availability() == YES) {
...
Trip* Driver::show_specefic_trips(vector <Trip*> trips, int id) {
    for (int num_of_trip = 0; num_of_trip < trips.size(); num_of_trip++) {
        if (trips[num_of_trip]->get_id() == id) {
            if (trips[num_of_trip]->get_availability() == YES) {
                return trips[num_of_trip];
                break;
            }
            else {
